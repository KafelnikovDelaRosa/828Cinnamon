    ...
    <script>
        ...
        function buttonEvents(){
            ...
            for(let i=0;i<codes.length;i++){
                let codeStock={code:'',name:'',required_stock:0,quantity:0,unit:'',from:'',to:'',cost:0,status:'in progress'};
                ...
                codeStock.to=year+'-'+month+'-'+(day+2);
                codeStock.cost=parseInt(cost[i].textContent*codeStock.required_stock);
                codeStock.status=status[i].textContent;
                total+=codeStock.cost;
                itemDict.push(codeStock);
            }
            console.log(itemDict,total);
            Swal.fire({
                    icon:'success',
                    title: `Bill of materials successfully created!`,
